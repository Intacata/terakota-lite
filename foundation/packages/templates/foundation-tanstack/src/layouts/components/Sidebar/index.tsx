...
export const defaultMenuItems: MenuItem[] = [
    {
        ...
        type: 'group',
        children: [
            { id: 'dashboard', title: 'Dashboard', path: '/home', icon: 'home' },
            /*@if:widgets*/
...
function MenuItemRow({ item, depth = 0, mini = false }: MenuItemRowProps) {
    ...
    const isActive    = !!item.path && routerState.location.pathname === item.path;
    ...
    const handleClick = () => {
        ...
        else if (item.path) { dispatch(setActiveItem(item.id)); navigate({ to: item.path }); }
    };
