...
### Fixed
- **Vite templates** — Removed `@emotion/babel-plugin` from `vite.config.ts` `react()` babel plugins. The package was listed but never installed, causing an `ERR_PACKAGE_NOT_FOUND` crash on first `npm run dev`. `jsxImportSource: '@emotion/react'` (which remains) is sufficient for MUI + Emotion; the babel plugin is an optional enhancement that requires explicit installation.
- Affects: `foundation-vite`, `foundation-vite-tailwind`. All other templates unaffected.
